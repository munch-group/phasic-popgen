...
def process_adjacent_trees(ts: tskit.TreeSequence,
    ...
    region : tuple of (float, float), optional
        Genomic region (start, end) to restrict processing to
    """
    sample_set = set(sample_subset)
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Example usage:
    python snp_descendants.py input.trees -o output.tsv
    python snp_descendants.py input.trees --samples samples.txt -o output.tsv
    python snp_descendants.py input.trees --samples samples.txt --max-offset 5 -o output.tsv
    python snp_descendants.py input.trees --region 1000000:2000000 -o output.tsv
...
Output format (TSV):
    ...
    parser.add_argument("--samples", type=Path, default=None,
                        help="File with sample identifiers (one per line). "
                             "If not provided, all samples in the TreeSequence are used.")
    ...
    parser.add_argument("--region", type=str, default=None,
                        help="Genomic region to process, as START:END (e.g. 1000000:2000000)")
    ...
    if args.samples:
        if args.verbose:
            print(f"Loading sample subset from {args.samples}...", file=sys.stderr)
        sample_subset = load_sample_subset(args.samples, ts)
    else:
        if args.verbose:
            print("Using all samples from TreeSequence...", file=sys.stderr)
        sample_subset = np.array(ts.samples(), dtype=np.int32)

    if args.verbose:
        print(f"  {len(sample_subset)} samples", file=sys.stderr)
    ...
    if args.region:
        try:
            ...
            region = (float(parts[0]), float(parts[1]))
        except (ValueError, IndexError):
            sys.exit(f"Error: Invalid region format '{args.region}'. Use START:END (e.g. 1000000:2000000)")

    # Process trees
    if args.verbose:
        region_str = f", region: {region[0]:.0f}-{region[1]:.0f}" if region else ""
        print(f"Processing tree pairs (max offset: {args.max_offset}{region_str})...", file=sys.stderr)

    process_adjacent_trees(ts, sample_subset, args.output, args.verbose,
                           max_offset=args.max_offset, region=region)
